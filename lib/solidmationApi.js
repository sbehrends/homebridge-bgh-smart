...
class Solidmation {
  ...
  async setDeviceStatus(DeviceID, mode) {
    ...
    delete device.endpointValues.currentTemp
    delete device.endpointValues.swingMode
    if (device.endpointValues.mode === 0) {
      device.endpointValues.fanMode = 255
      device.endpointValues.flags = 255
    }
    // console.log('===== Request for HVACSetModes =====')
    // TODO: When remote is off or no batteries Result: false
    console.log(`Will SET ${DeviceID}`, {
